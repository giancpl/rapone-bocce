...
import { assertBocceScore, bracketSize, firstRoundSlots, MAX_CONCURRENT_MATCHES, MAX_SCORE, MAX_TEAMS, MIN_WINNING_SCORE, nextMatchCoordinate, repechageCutoff, repechagePlan, repechagePlayoffWave, repechageRoundSlots, shuffleItems, tournamentEstimate, tournamentFormatAdvice } from "../../../lib/bracket";
...
function build(count: number, mode: Mode) {
  const size = bracketSize(count), shuffled = shuffleItems(teamNames(count)), slots = mode === "REPECHAGE" ? repechageRoundSlots(shuffled) : firstRoundSlots(shuffled), matches: Match[] = [];
...
function progressSimulatedRepechage(input: Match[], count: number, preliminaryIds: Set<string>) {
  ...
  const matches = copy(input);
  const targets = matches.filter(match => match.round === 1 && !match.winner && !match.a && !match.b).sort((a, b) => a.position - b.position);
  if (targets.length < qualifiers.length) return { matches: input, finalized: false, message: "Posti di ripescaggio non disponibili." };
  qualifiers.forEach((team, index) => { targets[index].a = team.name; });
...
export default function Simulation() {
  ...
  const generate = () => { const safe = count, safeMode = mode, generated = build(safe, safeMode); setMode(safeMode); setCount(safe); setMatches(generated); setPreliminaryIds(new Set(generated.filter(match => match.round === 1 && match.a && match.b).map(match => match.id))); setScores({}); setMessage(""); setRepechageFinalized(mode === "PRELIMINARIES" || !repechagePlan(safe).selections); setView("control"); };
