...
import { assertBocceScore, bracketSize, firstRoundSlots, MAX_CONCURRENT_MATCHES, MAX_SCORE, MAX_TEAMS, MIN_WINNING_SCORE, nextMatchCoordinate, repechageCutoff, repechagePlan, repechagePlayoffWave, shuffleItems, tournamentEstimate, tournamentFormatAdvice } from "../../../lib/bracket";
...
export default function Simulation() {
  ...
  useEffect(() => { if (!advice.repechageAvailable && mode === "REPECHAGE") setMode("PRELIMINARIES"); }, [advice.repechageAvailable, mode]);
  const generate = () => { const safe = count, safeMode = mode === "REPECHAGE" && !advice.repechageAvailable ? "PRELIMINARIES" : mode, generated = build(safe, safeMode); setMode(safeMode); setCount(safe); setMatches(generated); setPreliminaryIds(new Set(generated.filter(match => match.round === 1 && match.a && match.b).map(match => match.id))); setScores({}); setMessage(""); setRepechageFinalized(mode === "PRELIMINARIES" || !repechagePlan(safe).selections); setView("control"); };
  ...
  const finish = (match: Match, a: number, b: number) => { try { if (!match.a || !match.b) throw Error("Completa entrambe le coppie prima di registrare il risultato"); assertBocceScore(a, b); setMessage(""); const next = match.round === 0 ? copy(matches) : clearDescendants(matches, match); const item = next.find(value => value.id === match.id)!; Object.assign(item, { scoreA: a, scoreB: b, winner: a > b ? item.a! : item.b!, bye: false }); if (item.round > 0) place(next, item, item.winner!); const updated = settle(next, mode === "PRELIMINARIES" || repechageFinalized); if (mode === "REPECHAGE" && !repechageFinalized) { const preliminariesDone = [...preliminaryIds].every(id => updated.find(value => value.id === id)?.winner); if (item.round === 0) setMessage("Spareggio registrato. Controlla la classifica e conferma il passaggio successivo."); else if (preliminariesDone) setMessage("Preliminari completati. La classifica è pronta: controllala e conferma i ripescaggi."); } setMatches(updated); setScores(current => ({ ...current, [match.id]: { a: "", b: "" } })); } catch (error: any) { setMessage(error.message || "Risultato non valido"); } };
  const confirmRepechage = () => { const progress = progressSimulatedRepechage(matches, count, preliminaryIds); setMatches(progress.matches); setRepechageFinalized(progress.finalized); setMessage(progress.message || "Nessuna operazione disponibile."); };
  ...
  const plan = repechagePlan(count), estimate = tournamentEstimate(count), rankedLosers = candidates(matches, preliminaryIds), playoffMatches = matches.filter(match => match.round === 0), completedPreliminaries = [...preliminaryIds].filter(id => matches.find(match => match.id === id)?.winner).length;
  const unfinishedPlayoffs = playoffMatches.filter(match => !match.winner).length;
  const canConfirmRepechage = completedPreliminaries === plan.preliminaryMatches && unfinishedPlayoffs === 0 && !repechageFinalized;
  ...
  return <main className="simulation"><header className="adminHeader"><div><p className="kicker">Ambiente isolato</p><h1>Simulatore torneo</h1></div><Link className="minorButton" href="/admin">← Torna alla regia</Link></header><section className="simulationHero"><div><p className="kicker">Stesse regole, nessun dato reale</p><h2>Prova un torneo completo</h2><p>Bye, risultati e ripescaggi seguono le regole del torneo ufficiale.</p><small className="simulationEstimate">Con {count} coppie: {mode === "PRELIMINARIES" ? estimate.directMatches + " partite totali" : estimate.repechageMatches + (estimate.maxTieBreakMatches ? "–" + estimate.repechageMaxMatches : "") + " partite stimate"}. {advice.reason}</small></div><div className="simControls"><label className="teamStepper">Coppie<div><button type="button" aria-label="Diminuisci coppie" onClick={() => setCount(count - 1)}>−</button><input type="number" min="2" max={MAX_TEAMS} step="1" inputMode="numeric" value={countInput} onChange={event => setCountInput(event.target.value)} onBlur={() => setCount(count)} /><button type="button" aria-label="Aumenta coppie" onClick={() => setCount(count + 1)}>+</button></div></label><label>Formula<select value={mode} onChange={event => setMode(event.target.value as Mode)}><option value="PRELIMINARIES">Preliminari</option><option value="REPECHAGE" disabled={!advice.repechageAvailable}>Ripescaggi</option></select></label><button className="primaryButton" onClick={generate}>Genera prova</button>{matches.length > 0 && <button className="minorButton" onClick={() => setView("public")}>Vista pubblica</button>}{matches.length > 0 && <button className="textButton testReset" onClick={reset}>Azzera</button>}</div></section>{message && <p className="notice">{message}</p>}{mode === "REPECHAGE" && plan.selections > 0 && matches.length > 0 && !repechageFinalized && <section className="adminPanel simulationRepechage"><div className="sectionHeading"><div><p className="kicker">Decisione dell’admin</p><h2>{playoffMatches.length ? "Spareggi per la parità" : completedPreliminaries === plan.preliminaryMatches ? "Classifica pronta" : "In attesa dei preliminari"}</h2></div><span className="muted">{completedPreliminaries + " di " + plan.preliminaryMatches + " preliminari conclusi"}</span></div><p className="muted">Il sistema prepara la graduatoria per differenza punti e punti segnati. Nulla viene applicato finché non controlli e confermi.</p>{rankedLosers.length > 0 && <div className="repechageRanking">{rankedLosers.map((team, index) => <div key={team.name}><span>#{index + 1}</span><b>{team.name}</b><small>{team.scored}–{team.conceded} · diff. {team.difference}</small></div>)}</div>}{playoffMatches.length > 0 && <div className="playoffSummary">{playoffMatches.map(match => <div key={match.id}><span>{match.winner ? "Finita" : liveIds.has(match.id) ? "In corso" : "In attesa"}</span><b>{match.a} – {match.b}</b>{match.winner && <small>Vince {match.winner}</small>}</div>)}</div>}{canConfirmRepechage && <div className="repechageApproval"><div><b>{playoffMatches.length ? "Spareggi controllati" : "Classifica calcolata"}</b><small>{playoffMatches.length ? "Conferma per proseguire o assegnare i ripescati." : "Conferma per creare eventuali spareggi o assegnare i ripescati."}</small></div><button className="primaryButton mobilePrimaryAction" onClick={confirmRepechage}>{playoffMatches.length ? "Conferma e continua" : "Conferma classifica"}</button></div>}</section>}{!matches.length ? <div className="emptyState">Scegli numero di coppie e formula per generare una prova.</div> : <section className="adminPanel"><div className="sectionHeading"><div><p className="kicker">Tabellone di prova</p><h2>Regia incontri</h2></div><span className="muted">Puoi registrare o simulare i risultati e correggerli in seguito.</span></div><div className="simulationBoard">{rounds.map(round => <div className="simRound" key={round}><h3>{roundName(round, Math.max(0, ...rounds))}</h3>{matches.filter(match => match.round === round).map(match => { const score = scores[match.id] || { a: "", b: "" }; return <article className="simMatch manual" key={match.id}>{round === Math.max(0, ...rounds) && <small className="simPlacement">{match.position === 0 ? "Finale 1°/2° posto" : "Finale 3°/4° posto"}</small>}<input aria-label="Prima coppia" readOnly={match.round === 0} value={match.a || ""} placeholder="Da definire" onChange={event => updateMatch(match.id, "a", event.target.value)} /><strong>{match.winner ? (match.bye ? "Bye" : match.scoreA + " – " + match.scoreB) : liveIds.has(match.id) ? "In corso" : "—"}</strong><input aria-label="Seconda coppia" readOnly={match.round === 0} value={match.b || ""} placeholder="Da definire" onChange={event => updateMatch(match.id, "b", event.target.value)} />{match.a && match.b && !match.winner && <div className="simResult"><input aria-label="Punteggio prima coppia" type="number" min="0" max={MAX_SCORE} placeholder="0" value={score.a} onChange={event => setScores(current => ({ ...current, [match.id]: { ...score, a: event.target.value } }))} /><span>–</span><input aria-label="Punteggio seconda coppia" type="number" min="0" max={MAX_SCORE} placeholder="0" value={score.b} onChange={event => setScores(current => ({ ...current, [match.id]: { ...score, b: event.target.value } }))} /><button className="minorButton" onClick={() => finish(match, Number(score.a), Number(score.b))}>Registra</button><button className="textButton" onClick={() => simulate(match)}>Simula</button></div>}{match.winner && !match.bye && <button className="textButton simEdit" onClick={() => reopenResult(match)}>Modifica risultato</button>}</article>; })}</div>)}</div></section>}</main>;
}
