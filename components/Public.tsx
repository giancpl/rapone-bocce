...
function BracketGuide({ drawMode, matches }: { drawMode?: Tournament["drawMode"]; matches: Match[] }) { const active = matches.filter(match => match.status === "LIVE").length; const ready = matches.filter(match => ["READY", "WAITING"].includes(match.status)).length; return <div className="bracketGuide"><b>{drawMode === "REPECHAGE" ? "Formula con ripescaggi" : "Formula con preliminari"}</b><span>{drawMode === "REPECHAGE" ? "Dopo i preliminari, differenza punti e punti segnati formano la classifica. L’organizzazione la controlla e conferma prima di creare spareggi o nuovi abbinamenti." : "Le coppie senza avversario avanzano automaticamente al turno successivo."}</span><small>{active} in corso · {ready} in attesa</small></div>; }
...
function Bracket({ matches }: { matches: Match[] }) {
  ...
  return <div className="bracketExplorer">
    ...
    <label className="bracketMobilePicker"><span>Turno del tabellone</span><select value={selectedRound ?? ""} onChange={event => selectRound(Number(event.target.value))}>{rounds.map(round => <option value={round} key={round}>{roundName(round, totalRounds)} · {matches.filter(match => match.round === round).length} incontri</option>)}</select><small>{selectedRound === currentRound ? "Attuale · " : ""}{rounds.indexOf(selectedRound) + 1}/{rounds.length}</small></label>
