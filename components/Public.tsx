...
export default function Public({ initial, archived = false }: { initial: Tournament | null; archived?: boolean }) {
  ...
  const requestRegistration = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setPlayerOne("");
      setPlayerTwo("");
      setRegistrationMessage("Richiesta inviata.");
    } catch (reason: any) {
...
function MatchHighlights({ active, next, totalRounds, hasThirdPlace }: { active: Match[]; next: Match[]; totalRounds: number; hasThirdPlace: boolean }) {
  return <section className="boardSection matchHighlights" id="incontri"><div className="sectionHeading"><h2>Incontri</h2></div><div className="matchHub"><section className="matchLane activeLane"><div className="laneHeading"><h3>In corso</h3><b>{active.length}</b></div><div className="matchGrid">{active.map(match => <MatchCard key={match.id} match={match} prominent totalRounds={totalRounds} hasThirdPlace={hasThirdPlace} />)}{!active.length && <Empty text="Nessun incontro in corso." />}</div></section><section className="matchLane nextLane"><div className="laneHeading"><h3>In attesa</h3><b>{next.length}</b></div><div className="matchGrid">{next.slice(0, 4).map(match => <MatchCard key={match.id} match={match} totalRounds={totalRounds} hasThirdPlace={hasThirdPlace} />)}{!next.length && <Empty text="Nessun incontro in attesa." />}</div></section></div></section>;
}

function TournamentStats({ teams, live, next }: { teams: number; live: number; next: number }) {
  return <div className="tournamentStats" aria-label="Riepilogo torneo"><span><b>{teams}</b><small>coppie</small></span><span><b>{live}</b><small>in corso</small></span><span><b>{next}</b><small>in attesa</small></span></div>;
}
...
function BracketGuide({ drawMode }: { drawMode?: Tournament["drawMode"] }) { return <div className="bracketGuide"><b>{drawMode === "REPECHAGE" ? "Formula con ripescaggi" : "Formula con preliminari"}</b><span>{drawMode === "REPECHAGE" ? "La classifica dei preliminari determina i ripescaggi." : "I passaggi senza avversario sono automatici."}</span></div>; }
