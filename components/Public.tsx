...
export default function Public({ initial }: { initial: Tournament | null }) {
  ...
  if (!tournament) return <main className="landing"><div className="landingCard"><Brand /><Countdown /><p className="kicker">TORNEO DI BOCCE · 51° EDIZIONE</p><h1>Il torneo<br />è in preparazione.</h1><p>Il tabellone, i campi e i risultati compariranno qui in tempo reale.</p>{error && <p className="softError">{error}</p>}</div></main>;

  if (tournament.status === "SETUP") return <main className="landing"><div className="landingCard"><Brand /><Countdown /><p className="kicker">TORNEO DI BOCCE · 51° EDIZIONE</p><h1>{tournament.name}<br />sta per iniziare.</h1><p>Le coppie sono in fase di registrazione. Torna qui per seguire sorteggio e incontri.</p></div></main>;
