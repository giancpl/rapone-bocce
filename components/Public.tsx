...
function Bracket({ matches }: { matches: Match[] }) {
  ...
  return <div className="bracketScroll"><div className="bracket">{rounds.map((round, index) => <div className="bracketRound" key={round}><div className="roundLabel"><span>Turno {round}</span><small>{index === rounds.length - 1 ? "Finale" : `${matches.filter(match => match.round === round).length} incontri`}</small></div><div className="roundMatches">{matches.filter(match => match.round === round).map(match => <MatchCard key={match.id} match={match} bracket />)}</div></div>)}</div></div>;
}
